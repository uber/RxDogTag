 ...
 * @param <T> The type
 */
final class JavaDogTagSubscriber<T> implements FlowableSubscriber<T>, LambdaConsumerIntrospection {

  private final Throwable t = new Throwable();
  private final JavaRxDogTag.Configuration config;
  private final Subscriber<T> delegate;

  JavaDogTagSubscriber(JavaRxDogTag.Configuration config, Subscriber<T> delegate) {
    this.config = config;
