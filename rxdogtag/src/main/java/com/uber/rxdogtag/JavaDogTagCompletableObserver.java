 ...
 * {@link CompletableObserver#onSubscribe(Disposable) onSubscribe()} or {@link
 * CompletableObserver#onComplete()} onComplete()} methods and route them through the more specific
 * {@link #onError(Throwable) onError()} in this class for better tagging.
 */
final class JavaDogTagCompletableObserver
    implements CompletableObserver, LambdaConsumerIntrospection {

  private final Throwable t = new Throwable();
  private final JavaRxDogTag.Configuration config;
  private final CompletableObserver delegate;

  JavaDogTagCompletableObserver(JavaRxDogTag.Configuration config, CompletableObserver delegate) {
    this.config = config;
