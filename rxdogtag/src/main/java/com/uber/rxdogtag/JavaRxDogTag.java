 ...
 */
public final class JavaRxDogTag {
          ...
          for (ObserverHandler handler : config.observerHandlers) {
            ...
            if (observerToCheck instanceof LambdaConsumerIntrospection) {
              if (!((LambdaConsumerIntrospection) observerToCheck).hasCustomOnError()) {
                ...
                return new JavaDogTagObserver(config, originalObserver);
              }
            }
          }
          ...
          for (ObserverHandler handler : config.observerHandlers) {
            ...
            if (subscriberToCheck instanceof LambdaConsumerIntrospection) {
              if (!((LambdaConsumerIntrospection) subscriberToCheck).hasCustomOnError()) {
                //noinspection unchecked
                return new JavaDogTagSubscriber(config, originalSubscriber);
              }
            }
          }
        ...
        (single, originalObserver) -> {
          for (ObserverHandler handler : config.observerHandlers) {
            ...
            if (observerToCheck instanceof LambdaConsumerIntrospection) {
              if (!((LambdaConsumerIntrospection) observerToCheck).hasCustomOnError()) {
                //noinspection unchecked
                return new JavaDogTagSingleObserver(config, originalObserver);
              }
            }
          }
          return originalObserver;
        ...
        (maybe, originalObserver) -> {
          for (ObserverHandler handler : config.observerHandlers) {
            ...
            if (observerToCheck instanceof LambdaConsumerIntrospection) {
              if (!((LambdaConsumerIntrospection) observerToCheck).hasCustomOnError()) {
                //noinspection unchecked
                return new JavaDogTagMaybeObserver(config, originalObserver);
              }
            }
          }
          return originalObserver;
    ...
    RxJavaPlugins.setOnCompletableSubscribe(
        (completable, originalObserver) -> {
          for (ObserverHandler handler : config.observerHandlers) {
            ...
            if (observerToCheck instanceof LambdaConsumerIntrospection) {
              if (!((LambdaConsumerIntrospection) observerToCheck).hasCustomOnError()) {
                return new JavaDogTagCompletableObserver(config, originalObserver);
              }
            }
          }
          return originalObserver;
  ...
  static void reportError(
    ...
    StackTraceElement s = JavaRxDogTag.extractStackElementTag(stackSource, config.ignoredPackages);
  ...
  public static final class Builder {
     ...
     */
    public void install() {
      JavaRxDogTag.installWithBuilder(new Configuration(this));
    }
  }
