 ...
 * @param <T> The type
 */
final class JavaDogTagObserver<T> implements Observer<T>, LambdaConsumerIntrospection {

  private final Throwable t = new Throwable();
  private final JavaRxDogTag.Configuration config;
  private final Observer<T> delegate;

  JavaDogTagObserver(JavaRxDogTag.Configuration config, Observer<T> delegate) {
    this.config = config;
